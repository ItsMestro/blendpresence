...
from .pypresence.pypresence import Presence, exceptions
...
def getRenderEngineStr():
    ...
    unique = [["PRMAN_RENDER", "Renderman"], ["LUXCORE", "LuxCore"]]  # Engines with unique names
        ...
        if bpi.isRendering:
            ...
            if bpi.renderedFrames > 0:
                ...
                if prefs.displayFrames:
                    ...
                    stateText = (
                        f"Frame {frameRange[0]} of {frameRange[1]} ({'{:.2f}%'.format(percent)})"
                    )
            else:
    ...
    iconSet: bpy.props.EnumProperty(
        ...
        items=(
            ...
            (
                "workspace",
                "Active Workspace",
                "Icon will change based on the workspace you're in (Modelling, Animation, etc)",
            ),
        ),
        default="mode",
    )
    ...
    detailsType: bpy.props.EnumProperty(
        ...
        items=(
            (
                "literal",
                "Literal",
                "Changes depending on what you're doing (ex. while rendering, it will display 'Rendering a project')",
            ),
            (
                "custom",
                "Custom",
                "A string that will display in the 'details' property. Two characters or longer",
            ),
        ),
        default="literal",
    )
    ...
    stateType: bpy.props.EnumProperty(
        ...
        items=(
            (
                "anim",
                "Frames Animated",
                "Displays the frame number that holds the last keyframe from all given actions",
            ),
            ("poly", "Polygon Count", "Display the total amount of objects in the current scene"),
            (
                "bone",
                "Bone Count",
                "Display the total amount of armature bones in the current scene",
            ),
            (
                "mat",
                "Material Count",
                "Display the total amount of materials in the current scene",
            ),
            ...
            (
                "active",
                "Active Object",
                "Display the name of the curent active object selected. If none is seleced then this will return nothing",
            ),
            (
                "frame",
                "Current Frame",
                "Display the current frame being viewed in the timeline. The text will also change if you are playing back an animation",
            ),
            (
                "size",
                "File Size",
                "Displays the file size of your current project file. If your project is not saved, nothing will display",
            ),
            (
                "custom",
                "Custom",
